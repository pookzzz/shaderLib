...
void main()
{
	...
	materialSample = SampleMaterial( uv, adsk_results_pass8, resolution.xy, fNormalScale );
    vec3 alpha = texture2D(adsk_results_pass8, uv).rgb;
	...
	float fSpec = pow(abs(fNDotH), 10.0) * fNDotL * 0.5;
