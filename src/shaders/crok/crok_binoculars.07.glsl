#version 120
// blur chromatic abberation mask
uniform float blur_chroma_mask, adsk_result_w, adsk_result_h;
uniform sampler2D adsk_results_pass6;

void main()
{
   vec2 coords = gl_FragCoord.xy / vec2( adsk_result_w, adsk_result_h );
   int f0int = int(blur_chroma_mask);
   vec4 accu = vec4(0);
   float energy = 0.0;
   ...
   for( int x = -f0int; x <= f0int; x++)
   {
      ...
      float anEnergy = 1.0 - ( abs(float(x)) / blur_chroma_mask);
      energy += anEnergy;
