...
uniform float minInput, maxInput;
uniform sampler2D front, depth, matte;
...
vec4 render(vec3 ro, vec3 rd, float rz)
{
    ...
    // add fog
    z_depth.rgb = mix(vec3(1.0).rgb, z_depth.rgb, exp(-pow(ray_length, 2.0)));
...
void main()
{
    ...
    if ( antialiasing )
  	{
        ...
        vec3 pos = camToWorldPos(screenToCamPos(gl_FragCoord.xy + aa_offset * aa_softness, zd));
    ...
    {
      // get the fragment world position (NB : z < 0)
      vec3 pos = camToWorldPos(screenToCamPos(gl_FragCoord.xy, zd));
      // get the ray dir to march along
      vec3 rd=normalize(pos-ro);
      // get the ray length to march along
      float rz=distance(pos,ro);
    ...
    //levels adjustments for the DepthMatte
    z_depth.rgb = min(max(z_depth.rgb - vec3(minInput), vec3(0.0)) / (vec3(maxInput) - vec3(minInput)), vec3(1.0));
