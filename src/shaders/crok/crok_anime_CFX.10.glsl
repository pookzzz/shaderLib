...
uniform sampler2D adsk_results_pass8, adsk_results_pass9;
...
void main() {
	...
	vec2 d = texture2D(adsk_results_pass8, xy * px).xy;

	if(length(d) == 0.0) {
		// No gradient at this point in the map, early out
		gl_FragColor = texture2D(adsk_results_pass9, xy * px);
		return;
	}
	...
	if(odd) {
		...
		a = texture2D(adsk_results_pass9, xy * px);
		steps = (sam - 1.0) / 2.0;
	} else {
	...
	for(float i = 0.0; i < steps; i++) {
		d = texture2D(adsk_results_pass8, xy * px).xy;
