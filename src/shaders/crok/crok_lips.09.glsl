#version 120
// load dollface and softened lip mask

uniform float adsk_result_w, adsk_result_h;
uniform sampler2D adsk_results_pass3, adsk_results_pass8;
...
float alpha(vec2 uv)
{
   return adsk_getLuminance(texture2D(adsk_results_pass8, uv).rgb);
}
...
void main(void)
{
  ...
  float sr=sin(0.0);
	...
	vec4 col = vec4(texture2D(adsk_results_pass3, uv.xy+gradient ).rgb, 1.0);
