...
uniform vec3  translate;               // {"label":["Offset x","Offset y","Offset z"],  "min":-3,   "max":3,    "step":0.01,    "default":[0,0,0],  "group":"Fractal", "group_label":"Offsets"}
...
vec3 Mandelbulb(vec3 w)
{
    ...
    vec3 z = w;
    vec3 c = mix(w, vec3(0.0), juliaFactor);
    vec3 d = w;
    float dr = 1.0;
    float r  = length(z);
    float md = 10000.0;
