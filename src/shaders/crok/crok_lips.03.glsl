...
void main() {
	...
	// Centre sample
	vec4 orig = texture2D(adsk_results_pass2, xy * px);
	a += g.x * orig;
	energy += g.x;
	g.xy *= g.yz;
	...
	for(int i = 1; i <= support; i += inc) {
		b = texture2D(adsk_results_pass2, (xy - vec2(0.0, float(i))) * px);
		c = texture2D(adsk_results_pass2, (xy + vec2(0.0, float(i))) * px);
