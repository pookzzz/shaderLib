...
vec2 caret_origin = vec2(78.5, 0.1);
...
void main(void)
{
	...
	// Harbour View Hotel
	d += H(r()); add(); d += A(r()); add(); d += R(r()); add(); d += B(r()); add(); d += O(r()); add(); d+= U(r()); add(); d+= R(r()); add(); space(); d += V(r()); add(); d += I(r()); add(); d += E(r()); add(); d += W(r());
  add(); space(); d += H(r()); add(); d += O(r()); add(); d += T(r()); add(); d += E(r()); add(); d += L(r());
