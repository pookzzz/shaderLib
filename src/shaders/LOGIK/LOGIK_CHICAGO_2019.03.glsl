...
vec2 layerA(vec2 uv, vec2 co, float t)
{
	...
	p = uv;
	float w;
	float v = guy(vec2(mod(uv.x - t + 2.0, 30.0) - 15.0, uv.y), t * 0.8 + 38.7);
	w = circuit(uv / 4.0).y * 4.0;
	//v = min(v, w);
	q = uv;
  q.x = mod(q.x + 11.11, 11.11 * 2.) - 11.11;
  v = min(v, gear(rotate(q + vec2(1.9, 4.4), re(t) * .2 + .18)));
  v = min(v, gear(rotate(q + vec2(0.005, 4.), re(t) * -.2)));
	q = uv * 2.0;
...
vec2 layerA1(vec2 uv, vec2 co, float t)
{
	vec2 p, q;
	p = uv;
	float w;
  ...
  v = min(v, gear(rotate(q + vec2(0.005, 4.), re(t) * -.2)));
	...
	v = min(v, w);
	v = min(v, abs(uv.y + 1.25) - 0.1);
	v = smoothstep(0.0, aav*10., v);
	v = mix(0.0, v, step(abs(voronoi(uv * 0.4).y) - 2.0, 0.0));
	return vec2(0.0, v);
}

vec2 layerAA(vec2 uv, vec2 co, float t)
{
	vec2 p, q;
	p = uv;
	float w;
	float v = guy(vec2(mod(uv.x - t + 200.0, 300.0) - 15.0, uv.y), t * 0.6 + 0.2);
	w = circuit(uv / 4.0).y * 4.0;
	q = uv;
  q.x = mod(q.x + 11.11, 11.11 * 2.) - 11.11;
  v = min(v, gear(rotate(q + vec2(1.9, 4.4), re(t) * .2 + .18)));
...
vec2 layerC3(vec2 uv, vec2 co, float t)
{
	vec2 p, q;
	p = uv;
	float w;
...
vec2 layerD0(vec2 p, float t)
{
	float v;
  p.x = mod(p.x + 17., 35.) - 17.;
	v = pipeset(p / 5.0) * 5.0;
	v = smoothstep(0.0, aav*30., v);
	return vec2(0.0, v);
}
...
vec2 layerB(vec2 p, float t)
{
	...
	q.x = mod(q.x, 8.0) - 4.0;
	v = mech(q, tt).x * s;
	v = smoothstep(0.0, aav*20.0, v);
	return vec2(0.0, v);
}

vec2 layerImage1(vec2 p, float t)
{
	p.y += 1.0 * p1;
	p.y *= 2.0;
	p.x /= iResolution.x / iResolution.y;
	float v1 = texture2D(adsk_results_pass1, p*0.01).b;
	return vec2(0.0, v1);
}

vec2 layerImage2(vec2 p, float t)
{
	p.y += 1.0 * p2;
	p.y *= 2.0;
	p.x /= iResolution.x / iResolution.y;
	float v2 = texture2D(adsk_results_pass2, p*0.02).b;
	return vec2(0.0, v2);
}
...
void main(void)
{
	...
	// layer 0
	plane(ro, rd, normalize(vec3(0.0, 0.0, 1.0)), -50.0, r);
	r.x +=  250.0;
	v = layerD0(r.xy, iTime).y;
	col = mix(col, vec3(0.7), 1.0 - v);

	// layer 4
	plane(ro, rd, normalize(vec3(0.0, 0.0, 1.0)), -40.0, r);
	v = layerC3(r.xy, co, iTime).y;
	col = mix(col, vec3(0.6), v);

	// layer 0
	plane(ro, rd, normalize(vec3(0.0, 0.0, 1.0)), -30.0, r);
	v = layerD0(r.xy, iTime).y;
	col = mix(col, vec3(0.5), 1.0 - v);

	// layer Robot
	plane(ro, rd, normalize(vec3(0.0, 0.0, 1.0)), -20.0, r);
	v = layerB(r.xy, iTime).y;
	col = mix(col, vec3(0.3), 1.0 - v);

	// text layer
	plane(ro, rd, normalize(vec3(0.0, 0.0, 1.0)), -25.0, r);
	v = layerImage1(r.xy, iTime).y;
	col = mix(col, vec3(0.13, 0.13, 0.13), v);

	// layer 1
	plane(ro, rd, normalize(vec3(0.0, 0.0, 1.0)), -20.0, r);
	r.x +=  100.0;
	v = layerAA(r.xy, co, iTime).y;
	col = mix(col, vec3(0.08), 1.0 - v);

	// layer 3
	plane(ro, rd, normalize(vec3(0.0, 0.0, 1.0)), -15.0, r);
	v = layerC3(r.xy, co, iTime).y;
	col = mix(col, vec3(0.05), v);

	// logos layer
	plane(ro, rd, normalize(vec3(0.0, 0.0, 1.0)), -10.0, r);
	v = layerImage2(r.xy, iTime).y;
	col = mix(col, vec3(0.017, 0.017, 0.017), v);

	// layer 1
	plane(ro, rd, normalize(vec3(0.0, 0.0, 1.0)), -2.5, r);
	v = layerA1(r.xy, co, iTime).y;
	col = mix(col, vec3(0.01), 1.0 - v);

	// layer 0
	plane(ro, rd, normalize(vec3(0.0, 0.0, 1.0)), 2.5, r);
	v = layerD0(r.xy, iTime).y;
	col = mix(col, vec3(0.005), 1.0 - v);
