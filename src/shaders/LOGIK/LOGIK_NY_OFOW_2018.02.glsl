...
// based on https://www.shadertoy.com/view/MdfBRX by BigWIngs

uniform float adsk_result_w, adsk_result_h, adsk_time;
uniform sampler2D adsk_results_pass1;
vec2 res = vec2(adsk_result_w, adsk_result_h);
float iTime = adsk_time *.05;
...
void main()
{
    ...
    vec2 uv = gl_FragCoord.xy / res.xy; // 0 <> 1
		...
		float text = texture2D(adsk_results_pass1, uv).r;
    ...
    col += sat(rd.y)*vec3(.6, .5, .9);
		...
		vec3 f_c = screen(vec3(text), col);


	gl_FragColor = vec4(f_c, 0.);
}
