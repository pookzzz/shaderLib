...
float pScanline = 1.0;
float pSlowscan = 0.010;
float pVignettessoftness = 1.0;
float pVignettescale = 0.24;
float tv_tube_vignette_scale = 0.69;
float grain_opacity = 1.0;
float g_saturation = 0.0;
float pDistort = 0.03;
float timer = 0.85;
float speed  = 2.0;
float stripes_count = 1.28;
float Opacity= 1.0;
float bars_count = 3.9;
float tv_dots = 1.0;
float tv_dots_blend = 1.0;
float opacity_moire = 1.0;
float moire_scale = 0.15;
float Distort = 0.5;
float Scale = 1.06;
float tv_lines = 3.28;
float tv_lines_opacity = 0.5;
...
vec2 scandistort(vec2 uv) {
	...
	uv.x -= pDistort * mix(texture2D(adsk_results_pass5, vec2(uv.x, amount)).r * amount, amount, 0.9);
	return uv;
}
...
vec3 getVideo(vec2 uv)
{
	...
	vec3 video = vec3(texture2D(adsk_results_pass5,look));
	return video;
}
...
void main(void)
{
	...
	vec4 color = texture2D(adsk_results_pass5, crt_uv).rgba;
	...
	gl_FragColor = mix(color, mix(scanline_color, slowscan_color, 0.5), 0.05) * 1.35;
}
