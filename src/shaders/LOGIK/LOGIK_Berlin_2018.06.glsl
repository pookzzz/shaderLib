...
float vhs_res = 1.0;
float color_bleeding = 0.06;
float tape_wave = 4.0;
float tape_crease = 1.7;
float ac_beat = 1.37;
float yiq_blend = 0.06;
...
float v2random( vec2 uv ) {
  return texture2D( adsk_results_pass5, mod( uv, vec2( 1.0 ) ) ).x;
}

vec3 vhsTex2D( vec2 uv ) {
  if ( validuv( uv ) ) {
    return texture2D( adsk_results_pass4, uv ).rgb;
  }
...
void main( void )
{
  ...
  col *= 0.9 + 0.1 * texture2D( adsk_results_pass5, mod( uvn * vec2( .2, 1.0 ) + time * vec2( 0.97, 0.45 ), vec2( 1.0 ) ) ).xyz;
  col = saturate( col );
  // yiq
