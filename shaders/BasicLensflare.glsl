...
float noise1(float t) {
    return texture2D(iChannel0, vec2(t, 0.0)).x;
}
float noise2(vec2 t) {
    return texture2D(iChannel0, t).x;
}
...
vec3 lensflare(vec2 uv, vec2 pos) {
    ...
    f0 = f0 + f0 * (sin(noise1((pos.x + pos.y) * 2.2 + ang * 4.0 + 5.954) * 16.0) * 0.1 +
                    dist * 0.1 + 0.8);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    if (paramMove) {
        mouse.x = sin(iTime * paramSpeed / 100.0) * 0.5;
        mouse.y = sin(iTime * paramSpeed / 100.0 * 0.913) * 0.5;
    }

    vec3 color = vec3(1.4, 1.2, 1.0) * lensflare(uv, mouse.xy);
    color -= noise2(fragCoord.xy) * 0.015;
